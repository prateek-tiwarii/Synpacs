...
const getAllExportColumns = (): Record<string, boolean> => {
  return EXPORT_COLUMNS.reduce((acc, col) => {
    acc[col.id] = true;
    return acc;
  }, {} as Record<string, boolean>);
...
const getDefaultBookmarkDateRange = (): { startDate: string; endDate: string } => {
  const end = new Date();
...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const defaultDateRange = useMemo(() => getDefaultBookmarkDateRange(), []);
  ...
  useEffect(() => {
    ...
    setFileFormat('excel');
    setIncludeBookmarkNotes(true);

    if (exportType === 'bookmarks') {
      const range = getDefaultBookmarkDateRange();
      setStartDate(range.startDate);
      setEndDate(range.endDate);
      setSelectedColumns(getAllExportColumns());
      return;
    }

    setSelectedColumns(getDefaultExportColumns());
  }, [open, exportType]);
  ...
  const hasBookmarkDateRangeError =
    exportType === 'bookmarks' &&
    (!!startDate && !!endDate) &&
  ...
  const isDownloadDisabled =
    isExporting ||
    (exportType === 'bookmarks' && (!startDate || !endDate || hasBookmarkDateRangeError));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-150 max-h-[80vh] overflow-y-auto">
        ...
        <Tabs defaultValue="format" className="w-full">
          <TabsList className={`grid w-full ${exportType === 'bookmarks' ? 'grid-cols-4' : 'grid-cols-2'}`}>
            ...
            {exportType === 'bookmarks' && (
              <>
                ...
                <TabsTrigger value="date" className="flex-1">Date Range</TabsTrigger>
              </>
            )}
          </TabsList>
          ...
          {exportType === 'bookmarks' && (
            <>
              ...
              <TabsContent value="date" className="space-y-4 py-4">
                <div className="space-y-3">
                  <h3 className="text-sm font-medium">Study Date Range</h3>
                  <p className="text-xs text-muted-foreground">
                    Defaults to last 1 month. Export includes selected bookmarked cases within this date range.
                  </p>

                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <Label htmlFor="bookmark-export-start-date">Start Date</Label>
                      <Input
                        id="bookmark-export-start-date"
                        type="date"
                        value={startDate}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="bookmark-export-end-date">End Date</Label>
                      <Input
                        id="bookmark-export-end-date"
                        type="date"
                        value={endDate}
            ...
            <Button
              ...
              onClick={async () => {
                const payload = {
                  ...
                  startDate: exportType === 'bookmarks' ? startDate : undefined,
                  endDate: exportType === 'bookmarks' ? endDate : undefined,
